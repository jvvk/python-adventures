...
cdef class BCDPricer:
    ...
    def __init__(self, noofnames, k, delta, m, recoveryrate, discounter, copula, marginals):
        ...
        self.t = numpy.array([delta * (i+1) for i in range(0, m)])
        self.copula = copula

    def price(self, int noSimulations, includeAccruedPremium):
        cdef double plSum = 0, dlSum = 0, apSum = 0, defaulTime = 0.0
        cdef numpy.ndarray[double, ndim=1] uniformVariates
        cdef double dl,pl,ap,fairSpread
        simulations = []
        for i in range(0, noSimulations):
    ...
    cdef double computeDefaulTime(self, numpy.ndarray[double, ndim=1] variates):
        cdef numpy.ndarray[double, ndim=1] defaulttimes = numpy.array([self.marginals[i].inverseCdf(variates[i]) for i in range(0, self.noofnames)])
        defaulttimes.sort()
