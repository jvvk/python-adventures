...
class ImdbParser(object):
    ...
    # parses a lamnguage.list

    LANGUAGE_RE = "^(?P<title>[^\t]+?)\t+(?P<language>[\w-]+)([\s\w\(\)-]*)$"

    def generate_languages(self):
        """Parse movies.list and generate each movie title in turn."""
        lang_matcher = re.compile(self.LANGUAGE_RE)
        with self.open_list('language.list') as f:
            for line in f:
                match = lang_matcher.match(line)
                if match:
                    title = match.group('title').strip()
                    lang = match.group('language').strip()
                    yield (title, lang)
